...
//		Connection con=DBUtil.getConnection();
    ...
    public Phone getById(long id) {
        String sql = "SELECT phone.id as id, name, manufacturer, rom, ram, cpu, frontCamera, behindCamera, "
                + "os, battery, image, price, screen.id as screenId , size, technology, resolution "
                + "FROM phone, screen "
...
//		Connection con=DBUtil.getConnection();
    ...
    private void updateScreen(Screen screen, Connection con) throws SQLException {
...
//		Connection con=DBUtil.getConnection();
    ...
    public boolean delete(Phone phone) {
        ...
        ConnectionPool pool = ConnectionPool.getInstance();
        Connection con = pool.getConnection();
        String sqlPhone = "DELETE FROM phone WHERE id = ?";
        String sqlScreen = "DELETE FROM screen WHERE id = ?";
        PreparedStatement ps = null;
        try {
            con.setAutoCommit(false);
        ...
        try {
            con.setAutoCommit(false);
            ps = con.prepareStatement(sqlPhone);
            ps.setLong(1, phone.getId());
            ps.executeUpdate(); //Xoa Phone

            ps = con.prepareStatement(sqlScreen);
            ps.setLong(1, phone.getScreen().getId());
            ps.executeUpdate();
